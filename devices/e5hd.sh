...
if [ "$option" = "1" ]; then
  ...
  if [ "$optionubuntu" = "0" ]; then
    ...
    if [ "$ubuntuwipechannel" = "1" ]; then
      ...
      echo ""
      sleep 1
      echo "Please boot you E5 HD into bootloader/fastboot mode by pressing Power & Volume Up (+)"
      echo "and select fastboot mode"
      echo ""
      sleep 1
      echo -n "Is your E5 HD in bootloader/fastboot mode now? [Y/N] "; read bootloadermode
  ...
  if [ "$optionubuntu" = "1" ]; then
    ...
    if [ "$ubuntuwipechannel" = "1" ]; then
      ...
      echo ""
      sleep 1
      echo "Please boot you E5 HD into bootloader/fastboot mode by pressing Power & Volume +"
      echo "and select fastboot mode"
      echo ""
      sleep 1
      echo -n "Is your E5 HD in bootloader/fastboot mode now? [Y/N] "; read bootloadermode
    ...
    if [ "$optionubuntu" = "2" ]; then
      ...
      if [ "$ubuntukeepchannel" = "1" ]; then
        ...
        echo ""
        sleep 1
        echo "Please boot you E5 HD into recovery mode by pressing Power & Volume Up (+)"
        echo "and choose recovery"
        echo ""
        sleep 1
        echo -n "Is your E5 HD in recovery mode now? [Y/N] "; read recoverymode
        ...
        if [ "$ubuntukeepchannel" = "2" ]; then
          ...
          echo ""
          sleep 1
          echo -n "Is your E5 HD in recovery mode now? [Y/N] "; read bootloadermode
          if [ "$recoverymode"==Y -o "$recoverymode==y" ]; then
            echo ""
            echo "Detecting device"
            echo ""
