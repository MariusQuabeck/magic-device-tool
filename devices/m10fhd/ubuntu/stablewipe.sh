...
clear
  ...
  if [ "$developermode"==Y -o "$developermode"==y -o "$developermode"=="" ]; then
    ...
    then
      ...
      clear
      echo ""
      ubuntu-device-flash touch --bootstrap --device frieza --channel ubuntu-touch/stable/bq-aquaris-pd.en --recovery-image $HOME/.cache/magic-device-tool/recovery-frieza.img
      sleep 1
      echo ""
