...
clear
  ...
  if [ "$developermode"==Y -o "$developermode"==y -o "$developermode"=="" ]; then
    ...
    then
      ...
      echo "Please wait"
      sleep 6
      wget -c --quiet --show-progress --tries=10 http://people.ubuntu.com/~marius.quabeck/magic-device-tool/recoverys/recovery-frieza.img
      sleep 1
      clear
      echo ""
      ubuntu-device-flash touch --bootstrap --device frieza --channel ubuntu-touch/staging/ubuntu --recovery-image recovery-frieza.img
      sleep 1
      echo ""
