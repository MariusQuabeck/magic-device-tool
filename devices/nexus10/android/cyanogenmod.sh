...
then
  ...
  echo ""
  wget -c --quiet --show-progress --tries=10 http://people.ubuntu.com/~misterq/magic-device-tool/recoverys/twrp-3.0.2-0-manta.img
  sleep 1
  echo ""
  ...
  echo ""
  sleep 1
  echo "Choose 'Install' → select the manta-snapshot zip → press 'Add more Zips' to add"
