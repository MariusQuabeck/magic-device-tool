...
if [ "$option" = "1" ]; then
  ...
  if [ "$optionubuntu" = "1" ]; then
    ...
    if [ "$ubuntuwipechannel" = "1" ]; then
      ...
      echo ""
      sleep 1
      echo "Please boot you Nexus 7 into bootloader/fastboot mode by pressing Power & Vol -"
      echo ""
      sleep 1
      ...
      if [ "$ubuntuwipechannel" = "2" ]; then
        ...
        if [ "$bootloadermode"==Y -o "$bootloadermode==y" ]; then
          ...
          then
            ...
            fastboot reboot-bootloader
            sleep 6
            ubuntu-device-flash touch --channel ubuntu-touch/rc-proposed/ubuntu --device flo --bootstrap
            echo ""
            echo "Move to your device to finish the setup."
            sleep 1
            echo ""
    ...
    if [ "$optionubuntu" = "2" ]; then
      ...
      if [ "$ubuntukeepchannel" = "1" ]; then
        ...
        echo ""
        sleep 1
        echo "Please boot you Nexus 7 into recovery mode by pressing Power & Vol -"
        echo "and choose recovery"
        echo ""
        sleep 1
        echo -n "Is your Nexus 7 in recovery mode now? [Y/N] "; read recoverymode
        ...
        if [ "$ubuntukeepchannel" = "2" ]; then
          ...
          echo ""
          sleep 1
          echo -n "Is your Nexus 7 in recovery mode now? [Y/N] "; read bootloadermode
          if [ "$recoverymode"==Y -o "$recoverymode==y" ]; then
            echo ""
            echo "Detecting device"
            echo ""
            ...
            then
              ...
              sleep 1
              echo ""
              ubuntu-device-flash touch --channel ubuntu-touch/rc-proposed/ubuntu --device flo
              echo ""
              echo "Move to your device to finish the setup."
              sleep 1
              echo ""
    ...
    if [ "$option" = "3" ]; then
      ...
      if [ "$bootloadermode"==Y -o "$bootloadermode==y" ]; then
        ...
        then
          ...
          echo ""
          wget http://people.ubuntu.com/~misterq/phone/twrp-3.0.2-0-flo.img
          sleep 2
