...
then
  ...
  echo ""
  sleep 1
  wget -c --quiet --show-progress --tries=10 https://download.cyanogenmod.org/get/deb-snapshot.zip
  echo ""
  ...
  echo ""
  echo "  → CM 13 zip "
  adb push deb-snapshot.zip /sdcard/
  echo ""
  echo "  → gapps zip"
  ...
  echo ""
  sleep 1
  echo "Choose 'Install' → select the deb-snapshot zip → press 'Add more Zips' to add"
