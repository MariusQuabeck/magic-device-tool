...
then
  ...
  echo ""
  wget -c --quiet --show-progress --tries=10 -P $HOME/.cache/magic-device-tool/ http://people.ubuntu.com/~marius.quabeck/magic-device-tool/recoverys/twrp-deb.img
  sleep 1
  echo ""
  echo "Downloading Cyanogenmod 13.."
  echo ""
  sleep 1
  wget -c --quiet --show-progress --tries=10 -P $HOME/.cache/magic-device-tool/ https://download.cyanogenmod.org/get/deb-snapshot.zip
  echo ""
  echo "Downloading Open Gapps.."
  echo ""
  sleep 1
  wget -c --quiet --show-progress --tries=10 -P $HOME/.cache/magic-device-tool/ http://people.ubuntu.com/~marius.quabeck/magic-device-tool/gapps/open_gapps-arm-6.0-nano-20160811.zip
  sleep 2
  clear
  echo ""
  ...
  echo ""
  echo "  → CM 13 zip "
  adb push -p $HOME/.cache/magic-device-tool/deb-snapshot.zip /sdcard/
  echo ""
  echo "  → gapps zip"
  ...
  echo ""
  adb shell twrp install /sdcard/deb-snapshot.zip
  sleep 1
  echo ""
