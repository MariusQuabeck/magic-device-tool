#!/bin/bash
chmod +x . ./devices/nexus7grouper/android/*.sh
chmod +x . ./devices/generic/android/*.sh
clear
echo ""
echo "Nexus 7 2012 (WiFi) - grouper"
echo ""
echo ""
sleep 1
echo "Android: "
...
if [ "$option" = "1" ]; then
. ./devices/nexus7grouper/android/cyanogenmod.sh
elif [ "$option" = "2" ]; then
. ./devices/nexus7grouper/android/cyanogenmodwogapps.sh
elif [ "$option" = "3" ]; then
. ./devices/nexus7grouper/android/factoryimage.sh
elif [ "$option" = "4" ]; then
  . ./devices/nexus7grouper/android/twrp.sh
elif [ "$option" = "5" ]; then
  . ./devices/generic/android/backup.sh
elif [ "$option" = "6" ]; then
  . ./devices/nexus7grouper/android/bootloader.sh
elif [ "$option" = "7" ]; then
  . ./launcher.sh
else
  echo ""
